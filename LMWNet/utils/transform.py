...
class Compose(object):
    ...
    def __call__(self, image, label):
        for t in self.segtransform:
            if isinstance(t, torchvision.transforms.ToTensor) or isinstance(t, torchvision.transforms.Normalize):
...
class RandomEqualizeHist(object):
    def __init__(self, p=0.5):
        self.p = p
    ...
    def __call__(self, image, label):
        if random.random() < self.p:
            ...
            imgYUV = cv2.cvtColor(image, cv2.COLOR_RGB2YUV)
            channelYUV = cv2.split(imgYUV)
            channelYUV[0] = cv2.equalizeHist(channelYUV[0])  # change Y channel
            channels = cv2.merge(channelYUV)
            image = cv2.cvtColor(channels, cv2.COLOR_YUV2RGB)
...
class RandomGammaTransform(object):
    def __init__(self, gamma=1.0):
        ...
        self.table = np.array(table).astype('uint8')
